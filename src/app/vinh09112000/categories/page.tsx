...
export default function AdminCategories() {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (res.ok) {
        setNotification({ message: 'Đã xóa danh mục thành công!', type: 'success' });
        fetchCategories();
      } else {
        setNotification({ message: 'Lỗi: ' + (data.error || 'Không thể xóa danh mục'), type: 'error' });
      }
    } catch (error) {
      console.error('Lỗi khi xóa danh mục:', error);
      setNotification({ message: 'Lỗi kết nối khi xóa danh mục', type: 'error' });
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setNotification({ message: editingCategory ? 'Đã cập nhật danh mục!' : 'Đã thêm danh mục mới!', type: 'success' });
        ...
        setNotification({ message: 'Lỗi: ' + (errorData.error || 'Không thể lưu danh mục'), type: 'error' });
      }
    } catch (error) {
      setNotification({ message: 'Lỗi kết nối khi lưu danh mục', type: 'error' });
    }
  };
      ...
      {categoryToDelete && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl animate-in fade-in zoom-in duration-200 text-center">
            ...
            <p className="text-slate-500 mb-8">Nếu bạn xóa danh mục cha, các danh mục con và sản phẩm có thể bị ảnh hưởng. Bạn vẫn muốn tiếp tục?</p>
            <div className="flex gap-3">
              <button onClick={() => setCategoryToDelete(null)} className="flex-1 px-6 py-3 border border-slate-200 rounded-xl font-bold hover:bg-slate-50 transition-colors">Hủy</button>
              <button onClick={() => handleDelete(categoryToDelete)} className="flex-1 px-6 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-all shadow-lg shadow-red-200">Xóa ngay</button>
            </div>
          </div>
        </div>
      )}
